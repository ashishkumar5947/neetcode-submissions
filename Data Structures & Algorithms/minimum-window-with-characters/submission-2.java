class Solution {
    ...
    //         currFrequency.put(s.charAt(right), currFrequency.getOrDefault(s.charAt(right), 0) +
    //         1);
    ...
    public String minWindow(String s, String t) {
        ...
        int left = 0;
        Map<Character, Integer> currFrequency = new HashMap<>();
        int formed = 0, required = reqFrequency.size();

        for (int right = 0; right < s.length(); right++) {
            currFrequency.put(s.charAt(right), currFrequency.getOrDefault(s.charAt(right), 0) + 1);

            char c = s.charAt(right);
            if (reqFrequency.containsKey(c) && currFrequency.get(c).equals(reqFrequency.get(c))) {
                formed++;
            }
            ...
            while (formed == required) {
                ...
                c = s.charAt(left);
                currFrequency.put(c, currFrequency.getOrDefault(c, 0) - 1);
                if (reqFrequency.containsKey(c) && currFrequency.get(c) < reqFrequency.get(c)) {
                    formed--;
                }

                if (currFrequency.get(c) == 0)
                    currFrequency.remove(c);
